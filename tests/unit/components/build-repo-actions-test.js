...
test('it shows cancel button if canCancel is true', function (assert) {
  ...
  this.render();
  return assert.ok(component.$('a[title="Cancel build"]').length, 'cancel link should be visible');
});
...
test('it shows restart button if canRestart is true', function (assert) {
  ...
  this.render();
  assert.ok(component.$('a[title="Restart build"]').length, 'restart link should be visible');
});
